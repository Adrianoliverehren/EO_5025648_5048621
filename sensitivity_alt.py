...
if __name__ == '__main__':
    ...
    for i in range(2**13):
        ...
        if sim_dir != 'objectives_constraints.dat' and sim_dir != 'parameter_values.dat':
            ...
            if objective_constraints[2][i] <= 0:
                ...
                t_max_arr.append(objective_constraints[1][i])
    ...
    sorted_arrays = [v1_arr, v2_arr, v3_arr, t_impulse_arr]
    log_sensitivity_matrix = []
    for sorted_array in sorted_arrays:
        log_sensitivity_row = []
        derivative_arr = np.gradient(t_max_arr, sorted_array)

        # evaluate log sensitivity
        for x, f, dxdf in zip(sorted_array, t_max_arr, derivative_arr):
    ...
    ax1.plot(v1_arr, log_sensitivity_matrix[0], label=r'$\Delta V$ impulse in radial direction', lw=0, marker=".")
    ax1.plot(v2_arr, log_sensitivity_matrix[1], label=r'$\Delta V$ impulse in along-track direction', lw=0, marker=".")
    ax1.plot(v3_arr, log_sensitivity_matrix[2], label=r'$\Delta V$ impulse in cross-track direction', lw=0, marker=".")

    ax1.set_xlabel('Velocity Impulse [m/s]')
    ax1.set_ylabel('Log-sensitivity [-]')

    ax2.plot(t_impulse_arr, log_sensitivity_matrix[3], label=r'Time of impulse', c='magenta', lw=0, marker=".")
